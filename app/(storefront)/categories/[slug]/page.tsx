...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!prisma) return { title: "Category" };
  ...
  return {
    title: category.name,
    description: `Shop ${category.name} – ${count} product${count !== 1 ? "s" : ""} at Ecom Store.`,
  };
}
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <nav className="mb-6 text-sm text-[var(--foreground)]/70">
        ...
        <Link href="/categories" className="transition duration-200 hover:text-[var(--pink-500)]">Categories</Link>
        <span className="mx-2">/</span>
        <span className="font-medium text-[var(--pink-600)]">{category.name}</span>
      </nav>
